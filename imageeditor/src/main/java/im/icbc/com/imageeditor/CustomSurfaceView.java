package im.icbc.com.imageeditor;
...
public class CustomSurfaceView extends SurfaceView implements SurfaceHolder.Callback, Runnable, Handler.Callback {
    ...
    public CustomSurfaceView(Context context, String url, Bitmap bitmap, boolean s) {
        this(context, null);
        this.urlPath = url;
        this.bitmap = bitmap;
        saveBitmap = Bitmap.createBitmap(720, 1000, Bitmap.Config.ARGB_8888);
...
//        canvas.setBitmap(saveBitmap);
        ...
        if (state == 0) {
            ...
            arrow(startX, startY, stopX, stopY);
        } else if (state == 4) {
...
//            canvas.drawRect(targetRect, mpaint);
    ...
    public void arrow(int startX, int startY, int stopX, int stopY) {
        mpaint.setStyle(Paint.Style.FILL);
