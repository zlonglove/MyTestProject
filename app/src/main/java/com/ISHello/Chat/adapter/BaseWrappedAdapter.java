...
//                        获取该头部布局的位置,因为要考虑到空布局,当空布局和头部布局不能同时存在的时候，就返回-1
    ...
    public void addData(List<T> newData) {
        LogUtil.log("data.size()" + newData.size());
        addData(data.size(), newData);
    }
...
//                判断是否是第一次设置
    ...
    protected K createBaseViewHolder(View view) {
        LogUtil.log("createBaseViewHolder");
...
//                        总共个数为item个数和头部和尾部个数和更多加载view个数
        ...
        LogUtil.log("getItemCount" + count);
        return count;
    }
    ...
    private void processLoadMoreData() {
        ...
        if (data.size() > 0) {
            LogUtil.log("底部加载数据大小大于0");
            int size = data.size();
            LogUtil.log("loadStatus" + mLoadMoreView.getCurrentLoadingStatus());
